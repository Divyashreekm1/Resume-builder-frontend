...
export default function ManagerTemplate({ data }: TemplateProps) {
    return (
        <div className="flex w-full h-full bg-white text-gray-800 text-xs overflow-hidden">
                    ...
                    <h3 className="text-xs font-semibold uppercase tracking-wider border-b pb-1 mb-1">Skills</h3>
                     <ul className="text-xs list-disc list-inside">
                        {data.skills.map(skill => (
                            <li key={skill} className="text-[11px] mb-0.5">{skill}</li>
                        ))}
                    </ul>
                </div>
                 <div className="mt-4 space-y-2">
            ...
            <div className="w-2/3 p-4 overflow-y-auto">
                ...
                {data.experience.length > 0 && (
                    <Section title="Project Management Experience" icon={Briefcase}>
