...
export default function CommerceTemplate({ data }: TemplateProps) {
    return (
                    ...
                    <Section title="Projects" icon={FolderKanban}>
                        {data.projects.map(proj => (
                            <div key={proj.id} className="mb-3">
                                <h3 className="font-bold">{proj.name} {proj.link && <a href={`https://${proj.link}`} className="font-normal text-primary hover:underline">{proj.link}</a>}</h3>
