                    ...
                    <Section title="Projects" icon={FolderKanban}>
                        {data.projects.map(proj => (
                            <div key={proj.id} className="mb-2">
                                <h3 className="font-bold text-sm flex items-center">{proj.name} {proj.link && <a href={`https://${proj.link}`} className="font-normal text-primary hover:underline ml-1"><LinkIcon className="inline h-3 w-3" /></a>}</h3>
