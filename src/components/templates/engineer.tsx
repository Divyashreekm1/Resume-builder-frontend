...
const Section = ({ title, icon: Icon, children }: { title: string; icon: React.ElementType, children: React.ReactNode }) => (
    <div className="mb-6">
        <h2 className="text-xl font-bold border-b-2 border-gray-300 pb-2 mb-3 flex items-center">
...
export default function EngineerTemplate({ data }: TemplateProps) {
    return (
        <div className="bg-white text-gray-800 p-8 font-serif w-full h-full text-sm">
            ...
            {data.description && (
                <p className="text-center mb-8 text-sm italic">{data.description}</p>
            )}
                    ...
                    <Section title="Projects" icon={FolderKanban}>
                        {data.projects.map(proj => (
                            <div key={proj.id} className="mb-3">
                                <h3 className="font-bold">{proj.name} {proj.link && <span className="font-normal text-xs text-blue-600 hover:underline">({proj.link})</span>}</h3>
                ...
                <Section title="Certificates" icon={Award}>
                    {data.certificates.map(cert => (
                        <div key={cert.id} className="flex justify-between items-baseline mb-1">
                            <p className="text-sm"><span className="font-bold">{cert.name}</span> - {cert.issuingBody}</p>
